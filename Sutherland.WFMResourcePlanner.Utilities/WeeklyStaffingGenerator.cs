...
{
    public class WeeklyStaffingGenerator
    {
            ...
            DayOfWeek weekStart = DayOfWeek.Monday)
        {
            ...
            {
                string col = ColumnIndexToLetter(i + 2);
            ...
            foreach (var lob in planLobs)
            {
                var lobSheet = lobSheets.FirstOrDefault(s => s["name"]?.ToString() == lob.Name);
                if (lobSheet == null) continue;
                var config = WeekHelper.GetWeeklyStaffingConfigForLobType(lob.BillingModel);

                string lobName = lob.Name;
